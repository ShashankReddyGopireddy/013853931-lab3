...
class UpcomingOrders extends Component {
  ...
  componentDidMount = () => {
    ...
    axios.post(rootUrl + "/orders/upcomingOrders", data).then(response => {
      ...
      if (response.status === 200) {
        ...
        console.log("user orders", this.state.userOrders);
      } else {
  ...
  render() {
    ...
    if (route) {
      UniqueOdrer = route.map((order, index) => {
        ...
        i = i + 1;
        return <UniqueOrders key={i} orderIndividual={order} />;
      });
      return (
        <div>
          <Navbar />
